...
static double ex_rep_value;  // 期待値
...
| 多点交叉 |
\**********/
double mp_crossover(int cal1,int cal2)
{
    double chromosome1[20],chromosome2[20]; // 染色体2進数変換後格納用
    double crossovers;  // 交叉配列一時格納用
    double base; // 10進数変換用
    ...
    for(i = 0; i < 9;i++)
    {
        if(i < 4)
        {
            crossovers = chromosome1[i+3];
            chromosome1[i+3] = chromosome2[i+3];
            chromosome2[i+3] = crossovers;
        }else{
            crossovers = chromosome1[i+8];
            chromosome1[i+8] = chromosome2[i+8];
            chromosome2[i+8] = crossovers;
        }
    }
...
| 突然変異 |
\**********/
int mutation(int cal)
{
    double chromosome[20]; // 染色体2進数変換後格納用
    double mutationed = 0; // 突然変異後
    double base = 0; // 10進数変換用
    int variation_p[3];// 変異点
    int i;  // loop counter
    ...
    while(i < 3)
    {
        variation_p[i] = rand() % 20;
        if(i > 0 && variation_p[i-1] == variation_p[i])
        {
        }else{
            i++;
        }
    ...
    for(i = 0; i < 3; i++)
    {
        chromosome[variation_p[i]] = !chromosome[variation_p[i]];
    }
    
    // 10進数で値を返す
    for(i = 0; i < 20; i++)
    {
        mutationed += pow((chromosome[i] * 2),base);
        base++;
    }
...
int main() 
{
    ...
    int mutation_p; // 突然変異確率
    int mutation_p2; // 突然変異染色体
    ...
    for(icrossover = 0; icrossover < 100; icrossover++)
    {
        ...
        if(crossover_p > 1)
        {
            ...
            // op_crossover(chromosome[choise_parents[0]][0],chromosome[choise_parents[1]][0]); // 一点交叉
            mp_crossover(chromosome[choise_parents[0]][0],chromosome[choise_parents[1]][0]); // 多点交叉
            natural_selection();
        ...
        if(mutation_p < 1)
        {
            chromosome[mutation_p2][0] = mutation(chromosome[mutation_p2][0]);
            printf("Do Mutation Generation : %d\n",icrossover);
        }
    }
