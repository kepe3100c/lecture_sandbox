...
int main()
{
  ...
  char *option[]={"chasen_25", "-F", "%m %H\n", NULL};
	...
	/* 入力ファイルオープン */
	fp_in = fopen("kadai_25.txt", "r");
	if (fp_in == NULL){
		fprintf(stderr, "Cannot open file: kadai_25.txt\n");
		return -1;
	}

	/* 出力ファイルオープン */
	fp_out = fopen("kadai_25_r.txt", "w");
	if (fp_out == NULL){
		fprintf(stderr, "Cannot open file: kadai_25_r.txt\n");
		return -1;
	}
