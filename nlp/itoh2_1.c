...
int main()
{
  ...
  char *cmd2 = "cut -f3 nlp.txt | tr '\r\n' '\n' | grep -v -e '^\s*$' | sed -e 's/ *$//g' | sed -e 's/^[ ]*/E /g' | sed -e 's/$/ B/g' | sort";  // バイグラム用、文頭と文末を設定
  char *cmd3 = "sort nlp_f.txt | uniq -c | sed -e 's/^[ ]*//g' > nlp_pri_02.txt";  // バイグラムの頻度算出用
  char *cmd4 = "sort -r -n bigram_pri.txt | head -n 30"; // 上位30件表示
  char *cmd5 = "sort -n bigram_pri.txt | grep -v 0 | head -n 30";  // 下位30件表示
  ...
  */
  ...
  while((fgets(field2,80,in_file)) != NULL)
  {
    lenBy = strlen(field2);
    for(i = 0; i < lenBy; i++)
    {
  ...
  if((in_file2 = fopen("nlp_pri_02.txt","r")) == NULL)
  {
    printf(" cannot open out pipe!!");
    exit(0);
  }
  ...
  while((fgets(field,80,in_file)) != NULL)
  {
    ...
    while((fgets(field2,80,in_file2)) != NULL)
    {
      ...
      // それを避ける条件式を記述、また、これは1ループで終わるため、フラグで管理
      if((strstr(priPhoneme2,priPhoneme) - priPhoneme2) == 0 && !(strlen(priPhoneme) == 1 && strlen(priPhoneme2) > 2))
      {
        ...
        fprintf(out_file,"%.0f％\t%s\n",(nowFreq2 / nowFreq) * 100,priPhoneme2);
      }else if((strstr(priPhoneme2,"E") - priPhoneme2) == 0 && i == 0){
        // printf("%d  %.0f　　　\t / %.0f　　　\t%s\n",i,nowFreq2,sumUni,priPhoneme2);
        // fprintf(out_file,"%.0f %.3f %s\n",nowFreq2,(nowFreq2 / sumUni),priPhoneme2);
        fprintf(out_file,"%.0f％\t%s\n",(nowFreq2 / sumUni) * 100,priPhoneme2);
        i++;
      }
    }
